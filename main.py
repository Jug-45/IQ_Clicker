...
changelog_content = [
    ...
    "v0.1.1 (Pre-Alpha)",
    "- Добавлен ChangeLog",
    "- Добавлено возможность сохранение прогресса (не автоматическое)",
    "- Добавлены настройки:",
    "  1. Настройка громкости",
    "  2. Настройка разрешение экрана (1920x1080, 1600x900, 1280x720)"
]
...
press_exit_2 = pygame.Rect(100, 50, 300, 100)
...
def update_ui_positions():
    ...
    base_rects = {
        ...
        'press_settings': pygame.Rect(920, 500, 400, 100),
        'press_exit_1': pygame.Rect(920, 650, 400, 100),
        'press_changelog': pygame.Rect(50, 780, 400, 100),
        'press_exit_2': pygame.Rect(100, 50, 250, 85),
        'press_click': pygame.Rect(650, 400, 300, 150),
        'press_Minig': pygame.Rect(100, 200, 250, 100),
        'press_Minig_1': pygame.Rect(600, 200, 350, 100),
        'press_upg': pygame.Rect(1200, 50, 300, 100),
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEWHEEL:
            if game_state == 'upgrades':
                ...
                max_changelog_scroll = max(0, len(changelog_content) * 30 - screen_height - 200)
    ...
    if game_state == 'menu':
        ...
        changelog_x, changelog_y = scale_pos(150, 810)
        ...
        screen.blit(version_display, (version_x - 120, version_y))
        ...
        resolution_text_x, resolution_text_y = scale_pos(440, 375)
        ...
        resolution_value_x, resolution_value_y = scale_pos(750, 375)
        ...
        screen.blit(my_font.render("Применить", True, "White"), (apply_x, apply_y))
        ...
        screen.fill((30, 30, 30))  # Темный фон для changelog

        # Заголовок
        changelog_title = my_font_huge.render("Change Log", True, "White")
        screen.blit(changelog_title, (700, 30))

        # Отображение содержимого changelog с прокруткой
        for i, line in enumerate(changelog_content):
            line_surface = my_font.render(line, True, "White")
            screen.blit(line_surface, (100, 150 + i * 30 - changelog_scroll_offset))
        ...
        screen.blit(version_display,
                    (BASE_HEIGHT - version_display.get_width() - 220, BASE_WIDTH - version_display.get_height() - 20))
    # Остальные состояния игры (minigames, upgrades, autoclicker, changelog) обрабатываются аналогично
        ...
        minigames_1_x, minigames_1_y = scale_pos(635, 225)
        screen.blit(minigames_1, (minigames_1_x, minigames_1_y))
        ...
        title_x, title_y = scale_pos(1150, 100 - upgrades_scroll_offset)
        screen.blit(upgrade_title, (title_x, title_y))

        for upgrade in upgrades:
            # Обновляем позицию улучшения
            upgrade.update_position(upgrades.index(upgrade), upgrades_scroll_offset)

            # Проверяем, видима ли кнопка на экране
            if 0 <= upgrade.rect.y <= screen_height:
                ...
                pygame.draw.rect(screen, upgrade.current_color, upgrade.rect)
                ...
                text_x, text_y = scale_pos(1160, 160 + upgrades.index(upgrade) * 100 - upgrades_scroll_offset)
                screen.blit(name_text, (text_x, text_y))
                screen.blit(cost_text, (text_x, text_y + 25))
                screen.blit(effect_text, (text_x, text_y + 50))
        ...
        title_x, title_y = scale_pos(1150, 100)

        screen.blit(autoclicker_title, (title_x, title_y))

        # Отрисовка улучшений автокликера с учетом прокрутки

        for i, upgrade in enumerate(autoclicker_upgrades):

            # Используем базовые координаты и масштабируем их

            base_x = 1150  # Такая же позиция по X как у обычных улучшений
            ...
            upgrade_rect = pygame.Rect(
                ...
                (base_y - autoclicker_scroll_offset) * (screen_height / BASE_HEIGHT),
            ...
            if upgrade_rect.y + upgrade_rect.height >= 0 and upgrade_rect.y <= screen_height:
                ...
                # Масштабируем позиции текста

                text_x = upgrade_rect.x + 10 * (screen_width / BASE_WIDTH)

                text_y = upgrade_rect.y + 5 * (screen_height / BASE_HEIGHT)

                screen.blit(name_text, (text_x, text_y))

                screen.blit(cost_text, (text_x, text_y + 25 * (screen_height / BASE_HEIGHT)))

                screen.blit(effect_text, (text_x, text_y + 50 * (screen_height / BASE_HEIGHT)))
        ...
        # Заголовок Change Log
        changelog_title = my_font_huge.render("Change Log", True, "White")
        title_x, title_y = scale_pos(700, 100 - changelog_scroll_offset)
        screen.blit(changelog_title, (title_x, title_y))

        # Отображение содержимого Change Log
        for i, line in enumerate(changelog_content):
            line_text = my_font.render(line, True, "White")
            line_x, line_y = scale_pos(200, 200 + i * 30 - changelog_scroll_offset)

            # Проверяем, видима ли строка на экране
            if 0 <= line_y <= screen_height - 200:
